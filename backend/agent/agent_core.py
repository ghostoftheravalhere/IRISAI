"""AgentCore: Main orchestrator for goal understanding, multi-step planning, tool execution loops, security policy validation, and natural response synthesis."""
...
class AgentCore:
    """Main Agent Core Orchestrator coordinating task planning, security gates, tool feedback loops, and natural response generation."""
    ...
    def process_goal(
        ...
        """Process user goal through planning, security policy evaluation, tool feedback loops, and response synthesis."""
        with self._lock:
            ...
            plan = self._planner.create_plan(goal, available_tools, context)
    ...
    def resume_task_with_confirmation(self, task_state: TaskState, confirmed: bool) -> AgentResult:
        ...
        with self._lock:
            if not confirmed:
                ...
                return AgentResult(False, "Action cancelled by user.", task_state, error_code="CANCELLED")
